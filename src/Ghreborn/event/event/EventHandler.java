...
public class EventHandler {
	 ...
	 */
	private Queue<Event2<?>> pendingAddition = new LinkedList<>();

	/**
	 * A list of all the events that need to be removed from the list of running events.
	 */
	private Queue<Event2<?>> pendingRemoval = new LinkedList<>();

	/**
	 * A list of the currently running events
	 */
	private Queue<Event2<?>> active = new LinkedList<>();
	 ...
	 */
	public <T> void submit(Event2<T> event) {
	...
	public <T> void stop(T attachment) {
		Predicate<Event2<?>> equalTo = event -> event.getAttachment().equals(attachment);
	...
	public <T> void stop(T attachment, String signature) {
		Predicate<Event2<?>> equalTo = event -> event.getAttachment().equals(attachment) && event.getSignature() != null && event.getSignature().equals(signature);
		active.stream().filter(equalTo).forEach(pendingRemoval::add);
		pendingAddition.stream().filter(equalTo).forEach(pendingRemoval::add);
	}
	...
	public void stop(String signature) {
		Predicate<Event2<?>> predicate = event -> event.getSignature() != null && event.getSignature().equals(signature);
	...
	public <T> boolean isRunning(T attachment, String signature) {
		Predicate<Event2<?>> running = event -> event.isAlive() && event.getAttachment().equals(attachment) && event.getSignature() != null
