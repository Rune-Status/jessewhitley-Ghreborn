...
public class PanelSettings {
	...
	public void executeCommand(String cmd) {
		...
		if (cmd.equalsIgnoreCase("Teleport All")) {
			...
			for (int i = 0; i < Config.MAX_PLAYERS; i++) {
				if (validClient(i)) {
					Client c = getClient(i);
					TeleportExecutor.teleport(c, new Position(x, y, z));
				}
			}
			...
			if (p.NPC_CHECKBOX.isSelected()) {
				for (int i = 0; i < Config.MAX_NPCS; i++) {
					if (validNpc(i)) {
						NPC n = getNpc(i);
						if (inList(""+n.npcType)) {
							n.animId = Integer.parseInt(p.NPC_ANIMATION_TEXT.getText());
		...
		if (p.ENTITY_LIST.getSelectedValue() != null) {
			...
			if (cmd.equalsIgnoreCase("Teleport Player")) {
				...
				if (area.equals("Custom Location")) {
					...
					if (tele != null) {
						TeleportExecutor.teleport(c, new Position(tele.getX(), tele.getY(), tele.getZ()));
	...
	public void playerCommand(String cmd, Client c) {
		...
		if (cmd.equals("Force Chat")) {
			c.forceChat(getInput(cmd, "Enter a message for the player to say."));
		}
