...
public abstract class MultiplayerSession implements MultiplayerSessionItemDistribution, MultiplayerSessionLog {
	...
	/**
	 * A list containing just two players. This list will be maintained 
	 * to ensure that no more than two players ever exist within the trade
	 * so that no mischief or glitching is occuring.
	 */
	protected List<Client> players = new ArrayList<>(2);
	
	/**
	 * A map of each trader and the items they have offered during the trade.
	 */
	protected Map<Client, List<GameItem>> items = new HashMap<>();
	
	/**
	 * A map containing a list of game items the player has before the trade starts
	 */
	protected Map<Client, List<GameItem>> presetItems = new HashMap<>();
	
	/**
	 * The stage this current session is on. See {@link ab.model.trade.MultiplayerSessionStage} for more information
	 * on stages.
	 */
	 ...
	 * @param players
	 */
	public MultiplayerSession(List<Client> players, MultiplayerSessionType type) {
		this.players = players;
		this.type = type;
		players.stream().forEach(player -> items.put(player, new ArrayList<GameItem>()));
	}
	
	public abstract void accept(Client player, Client recipient, int stage);
	
	public abstract boolean itemAddable(Client player, GameItem item);
	
	public abstract boolean itemRemovable(Client player, GameItem item);
	 ...
	 */
	public void addItem(Client player, GameItem item) {
		int id = item.getId();
		...
		for(Client p : items.keySet()) {
			if(p != null && p.ironman) {
				player.sendMessage("You cannot offer items to or as an ironman!");
				return;
			}
		}
	 ...
	 */
	public void removeItem(Client player, int slot, GameItem item) {
		int id = item.getId();
		int amount = item.getAmount();
		if (Objects.isNull(player) || Objects.isNull(item)) {
			return;
	...
	public void accept(Client player, int stageId) {
	...
	public void refreshItemContainer(Client player, Client other, int containerId) {
	 ...
	 */
	protected boolean presetListContains(Client player, int itemId, int amount) {
		if (!new GameItem(itemId, amount).isStackable()) {
			Optional<GameItem> op = presetItems.get(player).stream().filter
					(i -> i.getId() == itemId).findFirst();
			return op.isPresent();
		}
		Optional<GameItem> op = presetItems.get(player).stream().filter
				(i -> i.getId() == itemId && i.getAmount() >= amount).findFirst();
		return op.isPresent();
	}
	
	/**
	 * Determines if the offered item list contains items that
	 * the preset list doesn't contain.
	 * @param player	the player 
	 * @return			true if the offered items contains an item that the preset
	 * 					doesnt have
	 */	
	protected boolean inventoryContainsIllegalItem(Client player, GameItem i) {
		long amount;
		if (!i.isStackable()) {
			amount = presetItems.get(player).stream().filter(item -> item.getId() == i.getId()).count();
		} else {
			Optional<GameItem> op = presetItems.get(player).stream().filter(
					item -> item.getId() == i.getId()).findAny();
			if (!op.isPresent()) {
				return true;
			}
			amount = op.get().getAmount();
		}
		return player.getItems().getItemAmount(i.getId()) + this.getItemAmount(
				player, i.getId()) > amount;
	}
	
	/**
	 * Returns a list of players that are in this session. This list should
	 * be modified from outside the class. This is to prevent the creation
	 * of any technical game bugs.
	 * @return
	 */
	 ...
	 * @return the player
	 */
	public Client getOther(Client player) {
	 ...
	 */
	protected boolean containsItem(Client player, int itemId, int itemAmount) {
		if (!items.containsKey(player))
			return false;
		GameItem item = new GameItem(itemId, itemAmount);
		if (!item.isStackable()) {
			return items.get(player).stream().filter(i -> i.getId() == item.getId())
					.count() >= itemAmount;
		}
		for (GameItem i : items.get(player)) {
	 ...
	 * @return true if it contains the item, false if not.
	 */
	protected boolean containsItem(Client player, int itemId) {
	 ...
	 * @return
	 */
	protected int getItemAmount(Client player, int itemId) {
		int amount = 0;
	 ...
	 */
	protected GameItem getOverlappedItem(Client player) {
		Client recipient = getOther(player);
	 ...
	 */
	protected List<GameItem> getItems(Client player) {
