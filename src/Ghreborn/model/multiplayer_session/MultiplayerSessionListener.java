...
public class MultiplayerSessionListener {
	...
	/**
	 * Determines if the session we want to append to the list
	 * is appendable.
	 ...
	 */
	public boolean containsSessionInconsistancies(Client player) {
		List<MultiplayerSession> playerMultiplayerSessions = sessions.stream().
				filter(session -> session.getStage().getStage() > MultiplayerSessionStage.REQUEST
						&& session.getPlayers().contains(player)).collect(Collectors.toList());
		if (playerMultiplayerSessions.size() > 1) {
			playerMultiplayerSessions.forEach(session -> session.finish(
					MultiplayerSessionFinalizeType.DISPOSE_ITEMS));
	 ...
	 */
	public MultiplayerSession requestAvailable(Client sender, Client receiver, MultiplayerSessionType type) {
		for (MultiplayerSession session : sessions) {
	...
	public void removeOldRequests(Client player) {
		sessions.removeAll(Arrays.asList(sessions.stream().filter(session -> session.getPlayers().contains(player) &&
				session.getStage().getStage() == MultiplayerSessionStage.REQUEST/* && session.getStage().getAttachment()
					== player*/).toArray()));
	}
	 ...
	 */
	public MultiplayerSession getMultiplayerSession(Client player, MultiplayerSessionType type) {
		for (MultiplayerSession session : sessions) {
			if (session == null) {
				continue;
			}
	 ...
	 * @return the trade session
	 */
	public MultiplayerSession getMultiplayerSession(Client player) {
		for (MultiplayerSession session : sessions) {
			if (session == null) {
				continue;
			}
	...
	/**
	 * Determines if the player exists within any of the current trade
	 * sessions that exist.
	 * @param player The player that we're examining
	 * @return
	 */
	public boolean inSession(Player player, MultiplayerSessionType type) {
		return sessions.stream().anyMatch(session -> session.type == type && session.getPlayers().contains(player)
				&& session.getStage().getStage() > MultiplayerSessionStage.REQUEST);
	}
	
	public boolean inAnySession(Client player) {
		return sessions.stream().anyMatch(session -> session.getPlayers().contains(player)
				&& session.getStage().getStage() > MultiplayerSessionStage.REQUEST);
	}
