package Ghreborn.model.content.dailytasks;
...
public class DailyTasks {
	...
	public enum PossibleTasks {
		...
		ADDY_DRAGONS(TaskTypes.PVM, 50, "Kill 50 Addy Dragons"),
		RUNE_DRAGONS(TaskTypes.PVM, 50, "Kill 50 Rune Dragons"),
	...
	public static void assignTask(Client player) {
		if (player.dailyEnabled) {

			if(player.currentTask != null) {
				player.sendMessage("You already have a daily task. Type '::dailytask' to check your task!", 255);
				return;
			}
			
			if (player.dailyTaskDate == getTodayDate() && player.currentTask == null && player.completedDailyTask == true) {
				player.sendMessage("You already completed your daily task", 255);
				return;
			} else {
				if (player.completedDailyTask == false && player.dailyTaskDate != getTodayDate() && player.currentTask == null) {
					...
					player.sendMessage("New Daily Task: " + player.currentTask.message, 255);
				}
			}
			
		} else
			player.sendMessage("Go speak to the daily task master to enable daily tasks!", 255);
	}
	
	public static void complete(Client player) {
		if(player.currentTask == null)
			return;
		...
		if(player.totalDailyDone >= player.currentTask.amount) {
			...
			player.sendMessage("You have completed your daily task: "+ player.currentTask.name().toLowerCase() + ", take these rewards!", 255);
			...
			player.sendMessage("You've been rewarded with some blood money and a daily task crate.", 255);
			player.currentTask = null;
		}
	}
	
	public static void increase(Client player, PossibleTasks task) {
		if(player.currentTask == null)
			return;
		
		if (player.currentTask.equals(task)) {
			player.totalDailyDone++;
			player.sendMessage(player.totalDailyDone + "/"+player.currentTask.amount+" completed of your task.", 255);
			complete(player);
		}
	}
