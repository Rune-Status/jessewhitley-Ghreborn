...
public class MazchnaDialogue extends Dialogue {
	...
	public void execute() {
		switch (getNext()) {
		case 0:
			if(getPlayer().combatLevel<20){
				DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.ANGRY_1, "Do not waste my time peasent.","You need a Combat level of at least 20.");
				setNext(1);
				return;
		...
		case 2:
			if (getPlayer().getSlayer().getTask().isPresent()) {
				if (getPlayer().getSlayer().getTask().get().getLevel() > getPlayer().playerLevel[Skill.SLAYER.getId()]) {
					DialogueManager.sendNpcChat(getPlayer(), 402, Emotion.CALM, "You have been assigned a task you cannot complete.",
	...
	public boolean clickButton(int id) {
		...
		case DialogueConstants.OPTIONS_4_2:
			DialogueManager.sendClientChat(getPlayer(), Emotion.CALM, "I need an assignment.");
			setNext(5);
			break;
