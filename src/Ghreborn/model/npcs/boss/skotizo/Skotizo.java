package Ghreborn.model.npcs.boss.skotizo;
...
public class Skotizo extends SingleInstancedArea {
	...
	/**
	 * Client variables, start coordinates.
	 */
	...
	public Skotizo(Client player, Boundary boundary, int height) {
		super(player, boundary, height);
	}
	
	
	public int calculateSkotizoHit(Client attacker, int damage) {
		if (altarCount == 0) {
		...
		if (random == 1) {
			...
			while (!unique) {
				if ((altarMap.get(1) == "NORTH") && (altarMap.get(2)== "SOUTH") && (altarMap.get(3) == "WEST") && (altarMap.get(4) == "EAST")) {
					player.sendMessage("<col=FF7F00>Your hits do not effect Skotizo... Maybe I should kill some of the altars...</col>");
					break;
				}
			...
			if (SKOTIZO.HP < 225 && !demonsSpawned) {
				NPCHandler.npcs[SKOTIZO.index].forceChat("Gar mulno ful taglo!");
