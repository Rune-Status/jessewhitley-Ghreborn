...
public final class NpcDefinition {
	...
	public NpcDefinition(int id, String name, String description, int combatLevel, int size, boolean attackable,
			boolean aggressive, boolean retreats, boolean poisonous, int respawnTime, int maxHit, int hitpoints,
			int attackSpeed, int attackAnimation, int defenceAnimation, int deathAnimation, int attackBonus,
			int meleeDefence, int rangedDefence, int magicDefence) {
		this.id = id;
		this.name = name;
	...
	public static NpcDefinition get(int npcId) {
		if (npcId < 0 || npcId > DEFINITIONS.length) {
			return produce(npcId);
		}
		NpcDefinition def = DEFINITIONS[npcId];
		return def == null ? produce(npcId) : def;
	}

	private static NpcDefinition produce(int npcId) {
		return new NpcDefinition(npcId, "NPC #" + npcId, "", 1, 1, false, false, false, false, 1, 1, 1, 4, -1, -1, -1,
				0, 0, 0, 0);
	}
