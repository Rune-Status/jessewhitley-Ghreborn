...
public class Friends {
	...
	public void add(long name) {
		...
		getFriends().add(name);
		Client friend = (Client) PlayerHandler.getPlayerByLongName(name);
	...
	public void sendPrivateMessage(long name, byte[] packed) {
		...
		if (friend != null) {
			...
			if (send) {
				friend.getPA().sendPM(client.getNameAsLong(), (byte) client.getRights().getValue(), packed);
			}
		} else {
			send = false;
		}
	...
	public void sendList() {
		for (long l : getFriends()) {
			boolean online = false;
			Client friend = (Client) PlayerHandler.getPlayerByLongName(l);
