...
public class Potions {
	...
	public void handlePotion(int itemId, int slot) {
		...
		if (c.potionTimer.elapsed(1200)) {
			...
			case 2446:
				...
				break;
			case 3008:
				drinkEnergy(itemId, 3010, slot);
				break;
			case 3010:
				drinkEnergy(itemId, 3012, slot);
				break;

			case 3012:
				drinkEnergy(itemId, 3014, slot);
				break;
			case 3014:
				drinkEnergy(itemId, 229, slot);
				break;
			case 175:
	...
	public void drinkEnergy(int itemId, int replaceItem, int slot) {
		...
		c.playerItems[slot] = replaceItem + 1;
		c.getItems().resetItems(3214);
 		c.runEnergy += c.runEnergy * .20;
               	c.updateRequired = true;
               	if (c.runEnergy + (c.runEnergy * .20) > 100) {
                c.runEnergy = 100;
               	}
	}
