...
public class TeleportTablets {
	 ...
	 * @param item
	 */
	public static void operate(final Client player, int item) {
		for (TabType type : TabType.values()) {
			if (type.getTabId() == item) {
				final int x = type.getX();
				final int y = type.getY();
				Teleport tele = new Teleport(new Position(x, y, 0), TeleportType.TABLET);
				if (TeleportExecutor.canTeleport(player, tele)) {
					return;
				}
				player.teleporting = true;
				player.getItems().deleteItem2(type.getTabId(), 1);
