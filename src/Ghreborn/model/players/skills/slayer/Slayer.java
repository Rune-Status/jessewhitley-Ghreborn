package Ghreborn.model.players.skills.slayer;
...
public class Slayer {
	...
	public void createNewTask(int masterId) {
		SlayerMaster.get(masterId).ifPresent(m -> {
			...
			DialogueManager.sendNpcChat(player, masterId, Emotion.LAUGHING, "You have been assigned " + taskAmount + " " + task.get().getPrimaryName() + ".", "Come talk to me when you finish this task.");
			player.nextChat = -1;
			master = m.getId();
		});
	}
	...
	public void killTaskMonster(NPC npc) {
		...
		task.ifPresent(task -> {
			 String name = npc.definition().getName().toLowerCase().replaceAll("_", " ");
		...
		task.ifPresent(task -> {
			...
			if (chance == 0) {
				...
				switch(task.getPrimaryName()){
					...
					player.sendMessage("<col=DD5C3E>A superior foe has appeared...");
					break;
					case "cave crawler":
