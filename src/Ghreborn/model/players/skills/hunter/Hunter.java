...
import Ghreborn.model.players.skills.hunter.trap.Trap;
import Ghreborn.model.players.skills.hunter.trap.Trap.TrapState;
import Ghreborn.model.players.skills.hunter.trap.Trap.TrapType;
import Ghreborn.model.players.skills.hunter.trap.TrapProcessor;
import Ghreborn.model.players.skills.hunter.trap.TrapTask;
...
public final class Hunter {
	...
	public static void abandon(Player player, Trap trap, boolean logout) {
		...
		if(logout) {
			GLOBAL_TRAPS.get(player).getTraps().forEach(t -> {
				...
				Server.itemHandler.createGroundItem(player, t.getType().getItemId(), t.getObject().getX(), t.getObject().getY(), t.getObject().getHeight(), 1, player.getId());
			});
			...
			Server.itemHandler.createGroundItem(player, trap.getType().getItemId(), trap.getObject().getX(), trap.getObject().getY(), trap.getObject().getHeight(), 1, player.getId());
	...
	public static boolean claim(Player player, GlobalObject object) {
		...
		double percentOfXp = (trap.experience() *  (Config.HUNTER_EXPERIENCE) / 100) * 2.5;

		Arrays.stream(trap.reward()).forEach(reward -> player.getItems().addItem(reward.getId(), reward.getAmount()));

		player.getPA().addSkillXP((int) ((int) trap.experience() * (Config.HUNTER_EXPERIENCE) + (player.getItems().isWearingItem(10071) ? percentOfXp : 0)), 21);
		...
		if (randomGold == 2 && player.getItems().getItemCount(13326) == 0 && player.summonId != 13326) {
		...
		case 10033:
			if (randomGray == 25 && player.getItems().getItemCount(13324) == 0 && player.summonId != 13324) {
		...
		case 10008:
			if (randomRed == 15 && player.getItems().getItemCount(13323) == 0 && player.summonId != 13323) {
			...
			DailyTasks.increase((Client)player, PossibleTasks.RED_CHINCHOMPAS);
			break;
			
		case 11959:
			if (randomBlack == 8 && player.getItems().getItemCount(13325) == 0 && player.summonId != 13325) {
