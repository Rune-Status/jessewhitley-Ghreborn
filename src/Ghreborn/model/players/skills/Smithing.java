...
public class Smithing {
	...
	public Smithing(final Client c) {
		this.c = c;
	}
	...
	private static void CheckAddy(final Client c, final int level,
	...
	private static void CheckBronze(final Client c, final int level,
			final int amounttomake, final String type) {
	...
	private static void CheckIron(final Client c, final int level,
			final int amounttomake, final String type) {
		remove = 2351;
	...
	private static void CheckMith(final Client c, final int level,
			final int amounttomake, final String type) {
		remove = 2359;
	...
	private static void CheckRune(final Client c, final int level,
			final int amounttomake, final String type) {
		remove = 2363;
	...
	private static void CheckSteel(final Client c, final int level,
			final int amounttomake, final String type) {
		remove = 2353;
	...
	public static void doAction(final Client c, final int toadd, final int toremove,
		...
		c.animation(898);
		if (c.playerSkilling[c.playerSmithing] == true) {
			return;
		}
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (c.playerSkilling[c.playerSmithing] == false) {
					container.stop();
					return;
				}
				...
				if (c.getItems().getItemName(toadd).contains("bolt")) {
					...
					c.getItems();
					if (c.getItems().getItemName(toadd).contains("nail")) {
						c.getItems().addItem(toadd, 15);
					} else {
						c.getItems();
						if (c.getItems().getItemName(toadd).contains("arrow")) {
							c.getItems().addItem(toadd, 15);
						} else {
							c.getItems();
							if (c.getItems().getItemName(toadd).contains("knife")) {
								c.getItems().addItem(toadd, 5);
							} else {
								c.getItems();
								if (c.getItems().getItemName(toadd).contains("cannon")) {
