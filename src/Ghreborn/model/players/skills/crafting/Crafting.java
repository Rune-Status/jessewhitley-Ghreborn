...
public class Crafting {
	...
	public int[][] gems = { { 1623, 1727, 1, 50 }, { 1621, 1729, 27, 68 }, { 1619, 1725, 34, 85 },
		...
		for (int j = 0; j < gems.length; j++) {
			if (gems[j][0] == id) {
				if (c.playerLevel[Player.playerCrafting] >= gems[j][2]) {
					...
					c.getPA().addSkillXP(gems[j][3] * Config.CRAFTING_EXPERIENCE, Player.playerCrafting);
					if (gems[j][0] == 1631)
	...
	public void craftHides(int id) {
		for (int j = 0; j < amount; j++) {
			...
			c.getPA().addSkillXP(exp * Config.CRAFTING_EXPERIENCE, Player.playerCrafting);
		}
		resetCrafting();
	}
	...
	public void openLeather(int item) {
		if (item == 1745) {
			c.getPA().sendChatInterface(8880); // green dhide
			...
			c.getPA().sendChatInterface(8880); // blue
			c.getPA().sendFrame126("What would you like to make?", 8879);
