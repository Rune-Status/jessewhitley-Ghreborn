...
public class Enchantment {
	...
	private boolean hasRunes(Client player, int spellID) {
		switch (spellID) {
		case 1155:
	...
	public void enchantItem(Client player, int itemID, int spellID) {
			...
			if (player.getItems().playerHasItem(enc.getUnenchanted(), 1)) {
				if (hasRunes(player, spellID)) {
					if (getEnchantmentLevel(spellID) == enc.getELevel()) {
						...
						player.getPA().addSkillXP(enc.getXp() * 10, player.playerMagic);
						player.animation(enc.getAnim());
