...
import Ghreborn.model.content.SkillcapePerks;
import Ghreborn.model.players.Client;
import Ghreborn.model.players.skills.Skill;

public class FletchLogEvent extends Event2<Client> {

	private FletchableLog fletch;

	private int amount;

	public FletchLogEvent(Client attachment, int ticks, FletchableLog fletch, int amount) {
...
/*		case 70:
		...
		attachment.getPA().addSkillXP(fletch.getExperience() * (attachment.getRights().isIronman() ? 1 : Config.FLETCHING_EXPERIENCE), Skill.FLETCHING.getId());
		if (amount <= 0) {
			stop();
			return;
		}
