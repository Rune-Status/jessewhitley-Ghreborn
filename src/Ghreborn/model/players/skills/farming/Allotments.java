...
public class Allotments {
	...
	private static final int CLEARING_EXPERIENCE = 4;
	
	public Allotments(Client player2) {
		this.player = player2;
	}
	...
	public enum AllotmentFieldsData {
		...
		CATHERBY_NORTH(0, new Location[]{new Location(2805, 3466), new Location(2806, 3468), new Location(2805, 3467), new Location(2814, 3468)}, 2324),

		CATHERBY_SOUTH(1, new Location[]{new Location(2805, 3459), new Location(2806, 3461), new Location(2802, 3459), new Location(2814, 3460)}, 2324),

		FALADOR_NORTH_WEST(2, new Location[]{new Location(3050, 3307), new Location(3051, 3312), new Location(3050, 3311), new Location(3054, 3312)}, 2323),

		FALADOR_SOUTH_EAST(3, new Location[]{new Location(3055, 3303), new Location(3059, 3304), new Location(3058, 3303), new Location(3059, 3308)}, 2323),

		PHASMATYS_NORTH_WEST(4, new Location[]{new Location(3597, 3525), new Location(3598, 3530), new Location(3597, 3529), new Location(3601, 3530)}, 2326),

		PHASMATYS_SOUTH_EAST(5, new Location[]{new Location(3602, 3521), new Location(3606, 3522), new Location(3605, 3521), new Location(3606, 3526)}, 2326),

		ARDOUGNE_NORTH(6, new Location[]{new Location(2662, 3377), new Location(2663, 3379), new Location(2662, 3378), new Location(2671, 3379)}, 2325),

		ARDOUGNE_SOUTH(7, new Location[]{new Location(2662, 3370), new Location(2663, 3372), new Location(2662, 3370), new Location(2671, 3371)}, 2325);

		private int allotmentIndex;
		private Location[] allotmentLocation;
		private int farmerBelonging;

		AllotmentFieldsData(int allotmentIndex, Location[] allotmentLocation, int farmerBelonging) {
			this.allotmentIndex = allotmentIndex;
			this.allotmentLocation = allotmentLocation;
			this.farmerBelonging = farmerBelonging;
		}

		public static AllotmentFieldsData forIdLocation(Location Location) {
			for (AllotmentFieldsData allotmentFieldsData : AllotmentFieldsData.values()) {
				if (FarmingConstants.inRangeArea(allotmentFieldsData.getAllotmentLocation()[0], allotmentFieldsData.getAllotmentLocation()[1], Location) || FarmingConstants.inRangeArea(allotmentFieldsData.getAllotmentLocation()[2], allotmentFieldsData.getAllotmentLocation()[3], Location)) {
		...
		public Location[] getAllotmentLocation() {
			return allotmentLocation;
		}
	...
	public boolean waterPatch(int objectX, int objectY, int itemId) {
		final AllotmentFieldsData allotmentFieldsData = AllotmentFieldsData.forIdLocation(new Location(objectX, objectY));
		...
		if (!player.getItems().playerHasItem(FarmingConstants.RAKE)) {
			player.getDH().sendStatement("You need a seed dibber to plant seed here.");
			return true;
		}
	...
	public boolean clearPatch(int objectX, int objectY, int itemId) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				player.getPA().addSkillXP(Config.FARMING_EXPERIENCE * CLEARING_EXPERIENCE, player.playerFarming);
	...
	public boolean plantSeed(int objectX, int objectY, final int seedId) {
		...
		//	player.getDH().sendStatement("You need a farming level of " + allotmentData.getLevelRequired() + " to plant this seed.");
		//	return true;
	//	}
		...
		if (player.getItems().getItemAmount(allotmentData.getSeedId()) < allotmentData.getSeedAmount()) {
			player.getDH().sendStatement("You need atleast " + allotmentData.getSeedAmount() + " seeds to plant here.");
			return true;
		}
		player.startAnimation(FarmingConstants.SEED_DIBBING);
		farmingStages[allotmentFieldsData.getAllotmentIndex()] = 4;
		player.getItems().deleteItem(seedId, player.getItems().getItemSlot(seedId), allotmentData.getSeedAmount());
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				farmingTimer[allotmentFieldsData.getAllotmentIndex()] = Server.getMinutesCounter();
				//player.getPA().addSkillXP(Config.FARMING_EXPERIENCE * allotmentData.getPlantingXp(), player.playerFarming);
				container.stop();
			}
	...
	public boolean harvest(int objectX, int objectY) {
		...
		player.setSkilling(new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (!player.checkTask(task) || player.getItems().freeSlots() <= 0) {
					container.stop();
					return;
				}
		...
		CycleEventHandler.getSingleton().addEvent(player, player.getSkilling2(), 2);
		return true;
	}
	...
	/*public boolean putCompost(int objectX, int objectY, final int itemId) {
		...
		player.getItems().removeItem(itemId);
		player.getItems().addItem(new Item(1925));
	...
	public boolean inspect(int objectX, int objectY) {
		...
		if (farmingState[allotmentFieldsData.getAllotmentIndex()] == 2) {
			player.getDH().sendStatement("This plant is diseased. Use a plant cure on it to cure it,", "or clear the patch with a spade.");
			return true;
		} else if (farmingState[allotmentFieldsData.getAllotmentIndex()] == 3) {
			player.getDH().sendStatement("This plant is dead. You did not cure it while it was diseased.", "Clear the patch with a spade.");
			return true;
		}
		if (farmingStages[allotmentFieldsData.getAllotmentIndex()] == 0) {
			player.getDH().sendStatement("This is an allotment patch. The soil has not been treated.", "The patch needs weeding.");
		} else if (farmingStages[allotmentFieldsData.getAllotmentIndex()] == 3) {
			player.getDH().sendStatement("This is an allotment patch. The soil has not been treated.", "The patch is empty and weeded.");
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					if (farmingStages[allotmentFieldsData.getAllotmentIndex()] - 4 < inspectData.getMessages().length - 2) {
						player.getDH().sendStatement(inspectData.getMessages()[farmingStages[allotmentFieldsData.getAllotmentIndex()] - 4]);
						...
						player.getDH().sendStatement(inspectData.getMessages()[inspectData.getMessages().length - 2]);
					} else {
						player.getDH().sendStatement(inspectData.getMessages()[inspectData.getMessages().length - 1]);
					}
					container.stop();
				}
	...
	/*public boolean curePlant(int objectX, int objectY, int itemId) {
		...
		player.getItems().removeItem(new Item(itemId));
		player.getItems().addItem(new Item(229));
