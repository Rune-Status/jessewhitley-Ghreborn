package Ghreborn.model.players.combat.magic;
...
public class MagicRequirements extends MagicData {
	...
	public static boolean hasRunes(Client c, int[] runes, int[] amount) {
		for(int i = 0; i < runes.length; i++) {
			if(c.getItems().playerHasItem(runes[i], amount[i]) || c.getRights().getValue() > 0) {
				return true;
			}
		}
	...
	public static boolean checkMagicReqs(Client c, int spell) {
		if(c.usingMagic && Config.RUNES_REQUIRED) { // check for runes
			if((!c.getItems().playerHasItem(MAGIC_SPELLS[spell][8], MAGIC_SPELLS[spell][9]) && !wearingStaff(c, MAGIC_SPELLS[spell][8])) ||
				(!c.getItems().playerHasItem(MAGIC_SPELLS[spell][10], MAGIC_SPELLS[spell][11]) && !wearingStaff(c, MAGIC_SPELLS[spell][10])) ||
				(!c.getItems().playerHasItem(MAGIC_SPELLS[spell][12], MAGIC_SPELLS[spell][13]) && !wearingStaff(c, MAGIC_SPELLS[spell][12])) ||
				(!c.getItems().playerHasItem(MAGIC_SPELLS[spell][14], MAGIC_SPELLS[spell][15]) && !wearingStaff(c, MAGIC_SPELLS[spell][14]))){			
		...
		if(c.usingMagic && c.playerIndex > 0) {
			if(PlayerHandler.players[c.playerIndex] != null) {
				for(int r = 0; r < c.REDUCE_SPELLS.length; r++){	// reducing spells, confuse etc
					if(PlayerHandler.players[c.playerIndex].REDUCE_SPELLS[r] == MAGIC_SPELLS[spell][0]) {
						c.reduceSpellId = r;
		...
		if(c.usingMagic && Config.MAGIC_LEVEL_REQUIRED) { // check magic level
			if(c.playerLevel[6] < MAGIC_SPELLS[spell][1]) {
				c.sendMessage("You need to have a magic level of " +MAGIC_SPELLS[spell][1]+" to cast this spell.");
				return false;
			}
		}
		if(c.usingMagic && Config.RUNES_REQUIRED) {
			if(MAGIC_SPELLS[spell][8] > 0) { // deleting runes
				if (!wearingStaff(c, MAGIC_SPELLS[spell][8]))
					c.getItems().deleteItem(MAGIC_SPELLS[spell][8], c.getItems().getItemSlot(MAGIC_SPELLS[spell][8]), MAGIC_SPELLS[spell][9]);
			}
			if(MAGIC_SPELLS[spell][10] > 0) {
				if (!wearingStaff(c, MAGIC_SPELLS[spell][10]))
					c.getItems().deleteItem(MAGIC_SPELLS[spell][10], c.getItems().getItemSlot(MAGIC_SPELLS[spell][10]), MAGIC_SPELLS[spell][11]);
			}
			if(MAGIC_SPELLS[spell][12] > 0) {
				if (!wearingStaff(c, MAGIC_SPELLS[spell][12]))
					c.getItems().deleteItem(MAGIC_SPELLS[spell][12], c.getItems().getItemSlot(MAGIC_SPELLS[spell][12]), MAGIC_SPELLS[spell][13]);
			}
			if(MAGIC_SPELLS[spell][14] > 0) {
				if (!wearingStaff(c, MAGIC_SPELLS[spell][14]))
					c.getItems().deleteItem(MAGIC_SPELLS[spell][14], c.getItems().getItemSlot(MAGIC_SPELLS[spell][14]), MAGIC_SPELLS[spell][15]);
			}
		}
		return true;
	}
