...
public class AttackPlayer {
	...
	public static void applyPlayerHit(final Client c, final int i, final Damage damage) {
	...
	public static void applyPlayerMeleeDamage(Client c, int i, int damageMask, int damage, Hitmark hitmark) {
	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		switch (type) {
			default:
			case MELEE:
				c.getPA().addSkillXP((int) Math.ceil((damage * (c.getRights().isIronman() ? 4 : Config.MELEE_EXP_RATE))) / 3, 3);
				switch (c.fightMode) {
					case 0: // Accurate
						c.getPA().addSkillXP((int) Math.ceil(damage * (c.getRights().isIronman() ? 4 : Config.MELEE_EXP_RATE)), 0);
						break;
					case 1: // Block
						if (c.playerLevel[1] == 1) {
							c.getPA().addSkillXP((int) Math.ceil(damage * (c.getRights().isIronman() ? 4 : Config.MELEE_EXP_RATE)), 2);
						} else {
						c.getPA().addSkillXP((int) Math.ceil(damage * (c.getRights().isIronman() ? 4 : Config.MELEE_EXP_RATE)), 1);
						}
						break;
					case 2: // Aggressive
						c.getPA().addSkillXP((int) Math.ceil(damage * (c.getRights().isIronman() ? 4 : Config.MELEE_EXP_RATE)), 2);
					...
					case 3: // Controlled
						...
						for (int i = 0; i < 3; i++)
							c.getPA().addSkillXP((int) Math.ceil((damage * (c.getRights().isIronman() ? 4 : Config.MELEE_EXP_RATE)) / 3), i);// 1.3
						}
						break;
					}
				break;
			case RANGE:
				...
				switch (c.fightMode) {
					...
					case 2: // Rapid
						c.getPA().addSkillXP(
								damage * (c.getRights().isIronman() ? 4 : Config.RANGE_EXP_RATE), 4);
						break;
					case 1: // Block
						c.getPA().addSkillXP(
								damage * (c.getRights().isIronman() ? 4 : Config.RANGE_EXP_RATE), 1);
						c.getPA().addSkillXP(
								damage * (c.getRights().isIronman() ? 4 : Config.RANGE_EXP_RATE) / 2, 4);
						break;
					}
				break;
			case MAGE:
				c.getPA().addSkillXP((damage * (c.getRights().isIronman() ? 4 : Config.MAGIC_EXP_RATE)) / 3, 3);
				int magicXP = damage * (c.getRights().isIronman() ? 4 : Config.MAGIC_EXP_RATE);
				c.getPA().addSkillXP(magicXP, 6);
				break;
		}
	}

	public static void calculateCombatDamage(Client attacker, Client defender) {
		int damage = 0;
		int damage2 = -1;
		int damage3 = -1;
		int damage4 = -1;
	...
	public static void resetSpells(Client c) {
	...
	public static void attackPlayer(Client c, int i) {
		...
		for (int j = 0; j < Config.LUCKY_ITEMS.length; j++) {
			if (c.playerEquipment[c.playerWeapon] == Config.LUCKY_ITEMS[j]) {
				c.sendMessage("You cant use luckys in pvp", 255);
				return;
			}
		}
		c.lastSent = 0;
		...
		if (!Config.ADMIN_ATTACKABLE && o.getRights().isAdministrator()) {
			c.sendMessage("You cannot attack " + Config.SERVER_NAME + " administrators!");
			c.getCombat().resetPlayerAttack();
			return;
		} else if (!Config.ADMIN_ATTACKABLE && c.getRights().isAdministrator()) {
				...
				if (c.attackTimer <= 0) {
					...
					if (c.usingSpecial && !c.usingMagic) {
						...
						if (c.getCombat().checkSpecAmount(c.playerEquipment[c.playerWeapon])) {
							...
							AttackPlayer.calculateCombatDamage(c, (Client)PlayerHandler.players[i]);
							c.getItems().updateSpecialBar();
						} else {
...
/*						double attack = c.getCombat().getMagicAccuracyRoll();
					...
					if (System.currentTimeMillis() - c.lastDamageCalculation > 1000) {
						calculateCombatDamage(c, (Client)PlayerHandler.players[i]);
