...
public class Food {
	...
	public static enum FoodToEat {
		...
		LEMON(2102, 2, "Lemon", 0, "Food", false), LIME(2120, 2, "Lime", 0,
				...
				"Manta Ray", 0, "Food", false), DARK_CRAB(11936, 22,
		...
		SEAWEED(403, 4, "Edible Seaweed", 0, "Food", false), MEAT_PIE(2327, 6,
				...
				//ANGLERFISH(13441, 22, "Anglerfish", 0, "Food", true);
	...
	public void eat(int id, int slot) {
		...
		boolean eatingKarambwam = lastEaten.isPresent() && !lastEaten.get().equals (FoodToEat.KARAMBWAN)
				&& f.equals(FoodToEat.KARAMBWAN) || c.lastClickedItem >= 6685 && c.lastClickedItem <= 6691;
		if ((c.foodDelay.elapsed(1500) || eatingKarambwam) && c.playerLevel[3] > 0) {
	...
	public void foodEffect(int id) {
		...
		case 1978:
		case 712:
			c.forceChat("Aaah, nothing like a nice cuppa tea!");
			break;
		case 13441:
			if(c.playerLevel[3] >= 99) {
				c.playerLevel[3] += c.playerLevel[3]/4.5;
			}
		case 1907:
			if (c.playerLevel[6] < 50) {
				c.playerLevel[6] = c.getPA().getLevelForXP(
						c.playerXP[6]) + 2;
			} else {
				c.playerLevel[6] = c.getPA().getLevelForXP(
						c.playerXP[6]) + 3;
			...
			if (c.playerLevel[0] < 5) {
				...
				c.playerLevel[0] = c.getPA().getLevelForXP(
						c.playerXP[0]) - 4;
			}
			if (c.playerLevel[1] < 4) {
				c.playerLevel[1] = 1;
			} else {
				c.playerLevel[1] = c.getPA().getLevelForXP(
						c.playerXP[1]) - 3;
			}
			if (c.playerLevel[2] < 4) {
				c.playerLevel[2] = 1;
			} else {
				c.playerLevel[2] = c.getPA().getLevelForXP(
						c.playerXP[2]) - 3;
			}
