...
public class Raids {
	...
	public void leaveGame(Player player) {
		...
		for(RaidRooms room : RaidRooms.values()) {
			if(room.getWay() == way) {
				player.getRaids().roomNames.remove(room.getRoomName());
				player.getRaids().roomPaths.remove(room.getDoor());
			}
		}
		currentRoom = 0;
		mobAmount=0;
	...
	public void giveReward() {
		player.getRaids().Hasreward = true;
	...
	public void giveRareReward() {
		...
		PlayerHandler.executeGlobalMessage("@red@" + player.playerName + " has received a rare item @red@"+ ItemCacheDefinition.forID(player.raidReward[0][0]).getName() + " from raids!");
	...
	public void handleMobDeath(int npcType) {
		...
		switch(npcType) {
			case OLM:
				Server.getGlobalObjects().add(new GlobalObject(-1, 3232, 5749, getHeight(player), 4, 10));
				Server.getGlobalObjects().add(new GlobalObject(29885, 3220, 5743, getHeight(player), 3, 10));
				Server.getGlobalObjects().add(new GlobalObject(29888, 3220, 5733, getHeight(player), 3, 10));
				Server.getGlobalObjects().add(new GlobalObject(29882, 3220, 5738, getHeight(player), 3, 10));
				Server.getGlobalObjects().add(new GlobalObject(30028, 3233, 5751, getHeight(player), 4, 10));
				Region.removeObject(30018, 3232, 5749, getHeight(player), 4, 10);
	...
	public void spawnNpcs(int currentRoom) {
		...
		switch(player.getRaids().roomNames.get(currentRoom)) {
			...
			case "olm":
				...
				NPCHandler.spawn(7555, 3223, 5733, height, -1, 500, 33, 272, 272,false); // left claw
				NPCHandler.spawn(7554, 3223, 5737, height, -1, 1600, 33, 272, 272,true); // olm head
				NPCHandler.spawn(7553, 3223, 5742, height, -1, 500, 33, 272, 272,false); // right claw
				...
				Server.getGlobalObjects().add(new GlobalObject(29881, 3220, 5738, getHeight(player), 3, 10));
				Server.getGlobalObjects().replace(new GlobalObject(30028, 3233, 5751, getHeight(player), 4, 10), new GlobalObject(30027, 3233, 5751, getHeight(player), 4, 10));
				Server.getGlobalObjects().replace(new GlobalObject(-1, 3232, 5749, getHeight(player), 4, 10), new GlobalObject(30018, 3232, 5749, getHeight(player), 4, 10));
				//Region.addObject(30018, 3232, 5749, getHeight(player), 4, 10);
				olm = true;
				mobAmount+=3;
				break;
		}
		...
		switch(objectId) {
			...
			case 30028:
				for (String username : player.getRaids().raidLeader.clan.activeMembers) {
					...
					for(RaidRooms room : RaidRooms.values()) {
						if(room.getWay() == way) {
							p.getRaids().roomNames.remove(room.getRoomName());
							p.getRaids().roomPaths.remove(room.getDoor());
						}
					}
