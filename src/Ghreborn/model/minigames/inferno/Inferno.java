...
public class Inferno extends Tzkalzuk {
	...
	public Inferno(Client player, Boundary boundary, int height) {
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer event) {
				...
				if(player.infernoWaveId != 0 && player.infernoWaveId < type.length)
					player.sendMessage("<col=DD5C3E>You are now on wave "+(player.infernoWaveId + 1)+" of "+type.length+".", 255);
					if(player.infernoWaveId == 68) {
						player.sendMessage("Relog if the boss instance does not start within the next 10 seconds.");
						//player.createTzkalzukInstance();
						initiateTzkalzuk();
					}
	...
	public void create(int type) {
		...
		player.infernoWaveId = player.infernoWaveId;
	...
	public void gamble() {
		...
		if (Misc.random(200) == 0) {
			 if (player.getItems().getItemCount(13225) == 0 && player.summonId != 13225) {
				// PlayerHandler.executeGlobalMessage("[<col=DD5C3E>PET<col=000000>] @cr20@<col=255> " + player.playerName + "</col> received a pet from <col=255>TzTok-Jad</col>.");
