...
public enum BountyHunterEmblem {
	 ...
	 */
	public static final Set<BountyHunterEmblem> EMBLEMS = Collections.unmodifiableSet(
			EnumSet.allOf(BountyHunterEmblem.class));
	...
	/**
	 * Compares the two emblems and determiens which of the two are 'better' based on the
	 * item identification value. 
	 */
	static final Comparator<BountyHunterEmblem> BEST_EMBLEM_COMPARATOR = (first, second)
			-> Integer.compare(first.itemId, second.itemId);
	
	/**
	 * Attempts to retrieve the best emblem the player has. The exclude parameter
	 * although may look out of place, allows us to determine if the TIER_10
	 * element should be excluded when determining the best. This is due to the 
	 * two scenarios in which we use this method. One when upgrading the emblem
	 * after killing a player, and the other when dropping the emblem on death.
	 * 
	 ...
	 */
	public static Optional<BountyHunterEmblem> getBest(Client player, boolean exclude) {
		List<BountyHunterEmblem> emblems = EMBLEMS.stream().filter(exclude(player, exclude))
	...
	/**
	 * Under certain circumstances we may want to exclude the tier 10 emblem from the possible stream of
	 * tiers we're trying to obtain. This is the case when dropping an emblem.
	 ...
	 */
	private static Predicate<BountyHunterEmblem> exclude(Client player, boolean exclude) {
		return emblem -> player.getItems().playerHasItem(emblem.getItemId()) &&
