package Ghreborn.model.minigames.bounty_hunter;
...
import Ghreborn.model.minigames.bounty_hunter.events.TargetDelay;
import Ghreborn.model.minigames.bounty_hunter.events.TargetSelector;
...
public class BountyHunter extends CycleEvent {
	...
	/**
	 * The amount of game cycles that must pass after a player has killed
	 * a target. This allows the player time to bank without being selected
	 * as a target, or given a target.
	 */
	...
	/**
	 * Each variable represents either the players current kills, or record in a specific
	 * feature. Each are only alive for the time of the session.
	 */
	...
	/**
	 * Creates a new BountyHunter object for the player. Every player will have thier own
	 * instance of a BountyHunter class.
	 * @param player	the player this class is created for
	 */
	public BountyHunter(Client player) {
		this.player = player;
	}
	...
	public void execute(CycleEventContainer container) {
		...
		if (hasTarget()) {
			Client target = (Client) PlayerHandler.getPlayer(this.target.getName());
		...
		if (targetState.isNone() && !CycleEventHandler.getSingleton().isAlive(targetSelector)
				&& targetSelector.isExecutable()) {
	...
	/**
	 * If a players target has died to another player it will reset his target allowing for a new target
	 */
	...
	/**
	 * The player skull should be updated when the worth of the player has changed. Worth it 
	 * determined by what it is they are risking in their inventory, and equipment.
	 */
	...
	/**
	 * The target interface should be updated when the distance between both players has changed,
	 * or the target has been removed. It should also be updated under a few other circumstances.
	 */
	public void updateTargetUI() {
		...
		if (target == null) {
			...
			if (targetState.isPenalized()) {
				...
				player.getPA().sendFrame126("<col=ffff00>------", 28005);
			}
		...
		Client playerTarget = (Client) PlayerHandler.getPlayer(target.getName());
	...
	/**
	 * Physically upgrades the player emblem in their inventory if they have one.
	 * How it works is the getBest function will determine the best owned 'emblem'
	 * in the players inventory and attempt to upgrade it. In the case that it cannot,
	 * because the player has no emblems, or has a tier 10 emblem it will return null,
	 * that is why we perform the {@code java.util.Objects.nonNull(Object)} check.
	 */
	public void upgradePlayerEmblem() {
		Optional<BountyHunterEmblem> emblem = BountyHunterEmblem.getBest(player, true);
		if (Objects.nonNull(emblem) && emblem.isPresent()) {
			player.getItems().deleteItem2(emblem.get().getItemId(), 1);
			player.getItems().addItem(emblem.get().getNextOrLast().getItemId(), 1);
			player.sendMessage("<col=255>Your emblem has been upgraded to "+Misc.capitalize(emblem.get().name().toLowerCase().replaceAll("_", " "))+".</col>");
		}
	}
	
	public void dropPlayerEmblem(Client killer) {
		if (killer != null) {
			Optional<BountyHunterEmblem> emblem = BountyHunterEmblem.getBest(player, false);
			//System.out.println("I'm working 1");
			...
			if (Misc.random(9) != 0 && killer.getBH().hasTarget() && hasTarget() && killer.playerName.equalsIgnoreCase(target.getName())) {
				if (emblem.isPresent()) {
					Server.itemHandler.createGroundItem(killer, emblem.get().getPreviousOrFirst().getItemId(), player.getX(), player.getY(),
							player.heightLevel, 1, killer.index);
					//System.out.println("I'm working 2");
				} else {
					if (Misc.random(3) != 0) {
						Server.itemHandler.createGroundItem(killer, BountyHunterEmblem.TIER_1.getItemId(), player.getX(), player.getY(),
								player.heightLevel, 1, killer.index);
						//System.out.println("I'm working 3");
					}
				}
			}
		}
	...
	public void teleportToTarget() {
		...
		if (player.playerIndex > 0 && target.index == player.playerIndex) {
	...
	public Client getPlayer(Boolean hasTarget) {
		for (int d = 0; d < Config.MAX_PLAYERS; d++) {
			if (PlayerHandler.players[d] != null) {
				Client o = (Client) PlayerHandler.players[d];
				if (o.playerName.equalsIgnoreCase(target.getName())) {
					return o;
				}
			}
		}
