...
import Ghreborn.model.content.dialogue.impl.KamfreenaDialogue;
	...
	public void handleDoor() {
		if(player.absX == 2847 && player.absY == 3540 || player.absX == 2847 && player.absY == 3541) {
			...
			if(player.getItems().playerHasItem(8851, 100)) {
				...
				if (current == -1) {
					player.start(new KamfreenaDialogue());
				} else {
					DialogueManager.sendNpcChat(player, 2461, Emotion.CALM,"You are currently in posession of a "+player.getItems().getItemName(current)+".",
							"The fee for re-entering the area is 100 tokens.");
							//player.nextChat = 627;
				}
			} else {
				DialogueManager.sendNpcChat(player, 2461, Emotion.CALM,"You need atleast 100 warrior guild tokens.", "You can get some by operating the armour animator.");
				//player.nextChat = 0;
			}
		}
	}
