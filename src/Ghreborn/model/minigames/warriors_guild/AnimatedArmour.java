...
public class AnimatedArmour {
	...
	public static void itemOnAnimator(final Client player, int itemId) {
		if((player.absX != 2851 || player.absY != 3537)
				&& (player.absX != 2857 || player.absY != 3537)) {
		...
		if(!player.getItems().playerHasItem(armour.getPlatebodyId(), 1)
				|| !player.getItems().playerHasItem(armour.getPlatelegsId(), 1)
		...
		CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
			...
			public void execute(CycleEventContainer event) {
				Server.npcHandler.spawnNpc(player, armour.getNpcId(), player.absX, player.absY + 1, 0,
						1, armour.getHealth(), armour.getMax(), armour.getAttack(),
						armour.getDefence(), true, true);
	...
	public static void dropTokens(Client player, int npcType, int x, int y) {
		...
		if(npc != null) {
			Server.itemHandler.createGroundItem(player, 8851, x, y,
					player.heightLevel, npc.getAmountOfTokens(), player.index);
			Server.itemHandler.createGroundItem(player, npc.getHelmId(), x, y,
					player.heightLevel, 1, player.index);
			Server.itemHandler.createGroundItem(player, npc.getPlatebodyId(), x, y,
					player.heightLevel, 1, player.index);
			Server.itemHandler.createGroundItem(player, npc.getPlatelegsId(), x, y,
					player.heightLevel, 1, player.index);
